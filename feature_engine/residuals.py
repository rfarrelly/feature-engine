...
def build_team_match_dataset(df, residual_definition="points"):
    ...
    df["HomeEP"] = expected_points(df["NoVigPH"], df["NoVigPD"])
    df["AwayEP"] = expected_points(df["NoVigPA"], df["NoVigPD"])
    ...
    return add_residual(team_df, definition=residual_definition)
...
def build_residual_dataset(path, residual_definition="points"):
    ...
    return build_team_match_dataset(
        calculate_market_probabilities(df),
