...
def identify_episodes(
    ...
    df["EpisodeSignal"] = np.select(
        [signal > positive_threshold, signal < negative_threshold],
        ["positive", "negative"],
        default=None,
    )
...
def summarize_episodes(df, z_column="ResidualZ_3"):
    ...
    summary = qualifying.groupby(
        GROUPS + ["EpisodeID", "EpisodeSignal"], as_index=False
    ).agg(
    ...
    peaks = (
        qualifying.groupby(GROUPS + ["EpisodeID", "EpisodeSignal"])[z_column]
        .agg(lambda x: x.max())
        .rename("PeakSignal")
        .reset_index()
    )

    negative = qualifying[qualifying["EpisodeSignal"] == "negative"]
    negative_peaks = (
        negative.groupby(GROUPS + ["EpisodeID"])[z_column]
        .min()
        .rename("PeakSignal")
        .reset_index()
    ...
    positive = qualifying[qualifying["EpisodeSignal"] == "positive"]
    positive_peaks = (
        positive.groupby(GROUPS + ["EpisodeID"])[z_column]
        .max()
        .rename("PeakSignal")
        .reset_index()
    ...
    peaks = pd.concat([positive_peaks, negative_peaks], ignore_index=True)
...
def measure_episode_outcomes(df, episodes, horizons=(1, 2, 3, 5)):
    ...
    for _, episode in episodes.iterrows():
        ...
        for horizon in horizons:
            values = np.asarray(future_values[:horizon], dtype=float)
...
def cluster_bootstrap_mean_ci(
    ...
    keys = list(clusters.itertuples(index=False, name=None))
    grouped = {
...
def cluster_bootstrap_signal_difference(
    ...
    for i in range(n_bootstrap):
        ...
        differences[i] = (
            np.mean(negative_values) - np.mean(positive_values)
    ...
    return (
        observed,
        np.quantile(differences, alpha / 2),
        np.quantile(differences, 1 - alpha / 2),
    )
...
def summarize_forward_residuals(episode_outcomes, horizons=(1, 2, 3, 5)):
    results = []
